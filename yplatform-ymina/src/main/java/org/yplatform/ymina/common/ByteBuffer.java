...
import org.yplatform.ymina.util.Stack;
import org.yplatform.ymina.v083v1.IoSession;
 ...
 * <li>It is hard to distinguish if the buffer is created from MINA buffer pool
 * or not. MINA have to return used buffers back to pool.
 * <li>
 ...
 *    <ul>
 *    	<li> You allocated a buffer,but didn't pass the buffer to any of two method above.</li>
 *    	<li> You called {@link #acquire()} to prevent the buffer from beging released.</li>
 *    </ul>
 ...
 *    to the buffer pool by default to prevent unexcepted memory leakage by default.
 ...
 * </pre>
 * 
 * NIO <tt>ByteBuffer</tt> is reallocated by MINA <tt>ByteBuffer</tt> behind the
 * scene if the encoded data is larger than 16 bytes. Its capacity will increase
 * by two times , and its limit will increase to the last position the String is
 * written.
 * 
 * @author yuanjinze
 *
...
public abstract class ByteBuffer {
	...
	private static final Stack[] heapBufferStacks = new Stack[] { new Stack(), new Stack(), new Stack(), new Stack(),
			new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(),
			new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(),
			new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(),
			new Stack(), new Stack(), new Stack(), new Stack(), };

	private static final Stack[] directBufferStacks = new Stack[] { new Stack(), new Stack(), new Stack(), new Stack(),
			new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(),
			new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(),
			new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(), new Stack(),
			new Stack(), new Stack(), new Stack(), new Stack(), };
	 ...
	 * This method tries to allocate direct buffer first, and then tries heap
	 * buffer if direct buffer memory is exhausted. Please use
	...
	/**
	 * Wraps the specified byte array into MINA heap buffer. Please note that
	 * MINA buffers are going to be pooled,and therefore there can be waste of
	 * memory if you wrap your byte array specifying <tt>offset</tt> and
	 * <tt>length</tt>
	 * 
	 * @param byteArray
	 * @param offset
	...
	/**
	 * Increases the internal reference count of this buffer to defer automatic
	 * release. You have to invoke {@link #release()} as many as you invoked
	 * this method to release this buffer.
	 * 
	...
	/**
	 * Returns <tt>true</tt> if and only if this buffer is returned back to the
	 * buffer pool when released.
	 * <p>
	 * The default value of this property is <tt>true</tt> if and only if you
	 * allocated this buffer using {@link #allocate(int)} or
	 * {@link #allocation(int,boolean)}, or <tt>false</tt> otherwise. (i.e.
	 * {@link #wrap(byte[])},{@link #wrap(byte[],int,int)}, and
	 * {@link #wrap(java.nio.ByteBuffer)};
	 * 
	 * @return
	 */
	 ...
	 * <p>
	 * The default value of this property is <tt>true</tt> if and only if you
	 * allocated this buffer using {@link #allocate(int)} of
	 * {@link #allocate(int,boolean)} or <tt>false</tt> otherwise. (i.e.
	 * {@link #wrap(byte[])}, {@link #wrap(byte[],int,int)} and
	 * {@link #wrap(java.nio.ByteBuffer)})
	 * 
	 * @param pooled
	 ...
	 * specified <code>decoder</code> and return it. This method reads until the
	 ...
	 *             if the speified string doesn't fit
	 * @throws CharacterCodingException
	 */
	 ...
	 * as a terminator. Please note that this method doesn't terminate with
	 * <code>NUL</code> if the input string is longer than <tt>fieldSize</tt>.
	 * 
	 * @param in
	 * @param fieldSize
	 ...
	 * @throws CharacterCodingException
	 */
	public abstract ByteBuffer putString(CharSequence in, int fieldSize, CharsetEncoder encoder)
			throws CharacterCodingException;
	...
	/**
	 * Fills this buffer with the specified value. This mehtod moves buffers
	 * position forward.
	 * 
	 * @param value
	 * @param size
	...
	/**
	 * Fills this buffer with the specified value. This mehtod does not change
	 * buffer position.
	 * 
	 * @param value
	 * @param size
	...
	private static class DefaultByteBuffer extends ByteBuffer {
		...
		public synchronized void acquire() {
			if (refCount <= 0) {
				throw new IllegalStateException("Already release buffer.");
			}
			refCount++;
		}
		...
		public ByteBuffer putString(CharSequence val, int fieldSize, CharsetEncoder encoder)
				throws CharacterCodingException {
			// TODO yjz
			return this;
		}
